...
namespace TagCompare.Util
{
    ...
    {
        private static readonly Dictionary<string, int> EncodingCodePageMap;
        ...
        {
            return EncodingCodePageMap.TryGetValue(encoding, out var codePage) ? Encoding.GetEncoding(codePage) : Default;
        }
        ...
        public static string GetSpecificCharacterSetForEncoding(Encoding encoding)
        {
            ...
            foreach (var codePage in EncodingCodePageMap.Where(codePage => codePage.Value == encoding.CodePage))
            {
                return codePage.Key;
            }

            throw new Exception(
                $"Unable to find specific character set value for encoding: {encoding.EncodingName} ({encoding.CodePage})");
        }
    }
}
